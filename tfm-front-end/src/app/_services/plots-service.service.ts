...
export class PlotsServiceService {
  ...
  getPlots(): Observable<Layer[]> {
    ...
    return this.httpClient.get<Layer[]>(`http://` + window.env.FRONT_ENDPOINT + `:8080/plots/owner/pepe`).
        pipe(
           map((data: Layer[]) => {
             console.log(data);
             return data;
    ...
    getPlotByCatastralReference(catastralReference: String): Observable<Layer> {
      ...
      // Variable "env" no existe en este momento, se definira dentro del "src/index.html"
      // de forma dinamica al lanzar el contenedor Docker.
      return this.httpClient.get<Layer>(`http://` + window.env.FRONT_ENDPOINT + `8080/plots/${catastralReference}`).
          pipe(
             map((data: Layer) => {
               console.log(data);
               return data;
    ...
    addPlot(owner:String, formData: FormData) {
      ...
      // Variable "env" no existe en este momento, se definira dentro del "src/index.html"
      // de forma dinamica al lanzar el contenedor Docker.
      this.httpClient.post<any>(`http://` + window.env.FRONT_ENDPOINT + `8080/plots/file/${owner}`, formData).subscribe(
        (res) => console.log(res),
        (err) => console.log(err)
      );
    }
